document.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightActiveNavLink() {
        const pagesToActivate = ["explore-vilhena", "desafios-culturais"];
    ...
    // --- LÓGICA ESPECÍFICA DA PÁGINA DE DESAFIOS ---
    const questButtons = document.querySelectorAll('.quest-card .btn');

    questButtons.forEach(button => {
        button.addEventListener('click', () => {
            const questCard = button.closest('.quest-card');
            const questTitle = questCard.querySelector('h4').textContent;
            alert(`Missão "${questTitle}" iniciada!\n\nSeu primeiro objetivo foi marcado no mapa da página "Roteiro Virtual". Boa sorte, explorador!`);
            // Em uma aplicação real, aqui haveria a lógica para registrar o início da quest para o usuário.
        });
    });
});
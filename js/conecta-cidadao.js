document.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightActiveNavLink() {
        const currentPageName = "conecta-cidadao"; // Nome da página atual
    ...
    // const newPostModal = document.getElementById('newPostModal'); // Se o modal de novo post estiver nesta página

    if (avanteVilhenaInsightsBtn) {
        avanteVilhenaInsightsBtn.addEventListener('click', () => {
    ...
    if (publishToForumBtn) {
        publishToForumBtn.addEventListener('click', () => {
            const forumUrl = publishToForumBtn.dataset.forumUrl; // forum-vilhenense.html
            ...
            if (proposalText.trim() && insightsText.trim() && forumUrl) {
                // Armazena os dados no localStorage para serem pegos pela página do fórum
                localStorage.setItem('newForumPostTitle', `Discussão da Proposta: ${proposalText.substring(0,50)}...`);
                localStorage.setItem('newForumPostContent', `**Proposta Original:**\n${proposalText}\n\n**Insights da IA:**\n${insightsText}`);
                localStorage.setItem('newForumPostCategory', 'Propostas Legislativas');
                
                closeModal(avanteVilhenaInsightsModal);
                window.location.href = forumUrl; // Redireciona para a página do fórum
            } else {
                alert("Não há proposta ou insights para publicar, ou a URL do fórum não foi definida.");
            }
        });
    }


    // Gráfico de Engajamento Cívico
    const civicCtx = document.getElementById('civicEngagementChart');
    if (civicCtx && typeof Chart !== 'undefined') {
        ...
        const chartColors = [
            ...
            '#059669', // Exemplo de cor verde
            '#D97706', // Exemplo de cor âmbar
            '#4F46E5'  // Exemplo de cor indigo
        ];

        new Chart(civicCtx, {
            type: 'bar',
            data: {
                labels: ['Projetos Visualizados', 'Votos em Consultas', 'Propostas Cidadãs', 'Problemas Reportados', 'Serviços Solicitados'],
                datasets: [{
                    label: 'Interações Mensais (Simulado)',
                    data: [1450, 980, 410, 560, 720], // Dados de exemplo

document.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightActiveNavLink() {
        const currentPageName = "comunidade";
    ...
    // --- Lógica Específica da Página Comunidade ---
    const suggestRecipeBtn = document.getElementById('suggestRecipeBtn');
    const submitRecipeSuggestionBtn = document.getElementById('submitRecipeSuggestion');
    const recipeModal = document.getElementById('suggestRecipeModal');
    const recipeResultDiv = document.getElementById('recipeSuggestionResult');
    const recipeOutputDiv = document.getElementById('recipeOutput');
    const recipeSpinner = recipeResultDiv?.querySelector('.loading-spinner');

    if (suggestRecipeBtn) {
        suggestRecipeBtn.addEventListener('click', () => openModal(recipeModal));
    }
    if (submitRecipeSuggestionBtn && recipeModal) {
        submitRecipeSuggestionBtn.addEventListener('click', async () => {
            const ingredientsEl = recipeModal.querySelector('#ingredientsText');
            const ingredients = ingredientsEl ? ingredientsEl.value : '';
             if (!ingredients.trim()) {
                if(recipeOutputDiv) recipeOutputDiv.textContent = "Por favor, liste os ingredientes.";
                if(recipeResultDiv) recipeResultDiv.classList.remove('hidden');
                return;
            }
            if (recipeSpinner) recipeSpinner.classList.remove('hidden');
            if (recipeResultDiv) recipeResultDiv.classList.remove('hidden');
            if (recipeOutputDiv) recipeOutputDiv.textContent = '';

            try {
                const resultText = await callGeminiAPI("Crie uma receita simples e saborosa utilizando principalmente os seguintes ingredientes locais: " + ingredients + ". Apresente o nome da receita, ingredientes e modo de preparo de forma clara.");
                typeWriterEffect(recipeOutputDiv, resultText);
            } catch (error) {
                if(recipeOutputDiv) recipeOutputDiv.textContent = "Erro ao sugerir receita: " + error.message;
            } finally {
                if (recipeSpinner) recipeSpinner.classList.add('hidden');
            }
        });
    }

    // Gráfico da Galeria de Arte
    const artCtx = document.getElementById('artGalleryChart');
    if (artCtx && typeof Chart !== 'undefined') {
        ...
        const chartColors = [ // Usando as cores do :root
            ...
            '#10B981', // Verde (Exemplo, pode ser uma var CSS)
            '#F59E0B', // Amarelo/Laranja (Exemplo)
            '#6366F1'  // Indigo (Exemplo)
        ];

        new Chart(artCtx, {
            type: 'doughnut',
            data: {
                labels: ['Pintura', 'Escultura', 'Fotografia', 'Arte Digital', 'Outros'],
                datasets: [{
                    label: 'Obras na Galeria (Simulado)',
                    data: [48, 20, 30, 15, 7], // Dados de exemplo
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: (context) => {
                                ...
                                return `${label}: ${new Intl.NumberFormat('pt-BR').format(value)}`;
                            }
                        }
                    }
                }
